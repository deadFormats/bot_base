...
from bot_base import exceptions
from bot_base.helpers import prompts
from bot_base.database.models import create_tables
...
if not os.path.isfile(os.path.join(BASEDIR, 'config.json')):
    patched_print("There is no 'config.json' file for this bot...")
    config_decision = inquirer.select(
        message="Cannot run without config.json file, how do you want to proceed? >>> ",
        choices=[
            Choice(name="Interactive Config Setup", value="new_config"),
            Choice(name="Manually input path to 'config.json' file", value="enter_path"),
            Separator(),
    ...
    elif config_decision == "enter_path":
        file_path = prompts.prompt_for_path()
